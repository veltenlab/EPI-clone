...
panel_file <- '/home/mscherer/cluster/project/Methylome/paper/HematoMethylome/github/EPI-clone/infos/panel_info_dropout_pwm.tsv'
...
rowinfo$SimpleCellType <- c("pre/pro-B"="pre/pro-B",
                       ...
                       "MkP 1"="MkP",
                       "MkP 2"="MkP",
                       "MPP2"="HSC/MPP1")[as.character(rowinfo$CellType)]
...
clust <- cutree(hclust(dist(frequencies, 'euclidean'), method='ward.D2'), 3)
clust_annotation <- c('2'='Myeloid Biased',
                      '1'='Balanced',
                      '3'='MegK Biased')
...
ComplexHeatmap::Heatmap(frequencies[, intersect(names(color_map), colnames(frequencies))],
...
```{r bias_umap}
seurat_clone <- CreateSeuratObject(t(frequencies))
seurat_clone <- FindNeighbors(seurat_clone,
                              features=rownames(seurat_clone),
                              dims=NULL,
                              assay='RNA')
u.map <- umap(frequencies)
to_plot <- data.frame(UMAP1=u.map[,1], UMAP2=u.map[,2], frequencies)
to_plot <- reshape2::melt(to_plot, id=c('UMAP1', 'UMAP2'))
ggplot(to_plot, aes(x=UMAP1, y=UMAP2, color=value))+geom_point()+plot_theme+scale_color_viridis(option = 'inferno', begin = 1, end=0)+NoGrid()+NoAxes()+facet_wrap(variable~., ncol=2)
```