...
epi_old <- epiclone(subset(full_seurat, Sample == "Old"), plotFolder = '.', tuneParams = F, 
                    ...
                    ncells.bigClone = 30, returnIntermediateSeurat = F)
...
small_clones <- sizes_old$x[sizes_old$freq<75]
...
small_clones <- sizes_young$x[sizes_young$freq<75]
...
ggplot(to_plot, aes(x=Sample, y=RelFreq, fill=x))+geom_histogram(stat='identity')+plot_theme+scale_fill_manual(values=cols, na.value = 'gray75')
print(paste('Old big clone', sum(sizes_old[!is.na(sizes_old$x), 'RelFreq']), 'Small clones:', sizes_old[is.na(sizes_old$x), 'RelFreq']))
...
ggplot(to_plot, aes(x=x, y=RelFreq, fill=Sample))+geom_histogram(stat='identity', position = 'dodge')+scale_fill_manual(values=c('Old'='gray75', 'Young'='gray25'))+plot_theme+xlab('Clone by size')+theme(axis.text.x = element_blank())+ylab('Frequency')
```
...
epimutations <- apply(clone_methylation_mat, 2, function(x){
  sum(x[row.names(subset(info[static_cpgs, ,drop=FALSE], subset=Type=='Methylated'))]==0)
})
...
epimutations <- data.frame(Epimutations=epimutations, TotalCounts=total_counts_mat, row.names = paste0('Clone', unique(Idents(epi_old_seurat))))
epimutations$Residuals <- lm(Epimutations~TotalCounts, data=epimutations)$residuals
...
# sub-sample to the same number of cells per cluster
rowinfo_young <- epi_young_seurat[[]]
ct_counts_young <- table(rowinfo_young$CellType)
ct_counts_old <- table(rowinfo$CellType)
# set.seed(314)
# rowinfo_sub <- do.call('rbind', lapply(unique(names(ct_counts_old)), function(ct){
#   rowinfo_sel <- rowinfo[rowinfo$CellType==ct, ]
#   return(rowinfo_sel[sample(1:nrow(rowinfo_sel), min(ct_counts_old[ct], ct_counts_young[ct])), ])
# }))
# rowinfo_sub <- rowinfo_sub[!is.na(rowinfo_sub$SimpleCellType), ]
...
epimutations <- data.frame(Epimutations=epimutations, TotalCounts=total_counts_mat, row.names = paste0('Clone', unique(Idents(epi_young_seurat))))
epimutations$Residuals <- lm(Epimutations~TotalCounts, data=epimutations)$residuals
...
rowinfo <- rowinfo[!is.na(rowinfo$SimpleCellType), ]
for(x in clone_counts$x){
  counts <- plyr::count(rowinfo[rowinfo_sub$Clone%in%x, 'SimpleCellType'])
  frequencies[x, counts$x] <- counts$freq
}
clone_sizes <- rowSums(frequencies, na.rm = TRUE)
frequencies[is.na(frequencies)] <- 0
frequencies <- apply(frequencies, 2, function(x)x/sum(x))
frequencies <- frequencies/rowSums(frequencies)
clust <- cutree(hclust(dist(frequencies, 'euclidean'), method='ward.D2'), 4)
...
ggplot(to_plot[!is.na(to_plot$CellType),], aes(x=seurat_clusters, y=RelFreq, fill=Sample))+geom_histogram(stat='identity', position='dodge')+
...
to_plot <- rbind(old_in_big_clone[1:20, ], young_in_big_clone[1:20, ])
ggplot(to_plot[!is.na(to_plot$CellType),], aes(x=seurat_clusters, y=RelFreq, fill=Sample))+geom_histogram(stat='identity', position='dodge')+
  plot_theme+scale_fill_manual(values=c('Old'='#d9d98e', 'Young'='#808054'))+xlab('Clones')+
...
epi_old_seurat$Clones <- paste0('Clone', Idents(epi_old_seurat))
write.csv(epi_old_seurat[[]], 'table_old.csv')
epi_young_seurat$Clones <- paste0('Clone', Idents(epi_young_seurat))
write.csv(epi_young_seurat[[]], 'table_young.csv')
```
