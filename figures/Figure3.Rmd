...
DimPlot(subset(larry,use), reduction = "cloneUMAP", group.by = "LARRY") + NoLegend() + scale_color_manual(values = cloneColors.here) + NoAxes() + ggtitle("LARRY Clone")
DimPlot(subset(larry,use), reduction = "cloneUMAP", group.by = "selected") + NoLegend() + NoAxes() + ggtitle("Selected by density") + scale_color_manual(values = c("TRUE" = "#AA0000", "FALSE" = "#555555"))
...
```{r clusterresult, echo=F, fig.width=4,fig.height=4.2}
    ...
    DimPlot(for_clustering, reduction="cloneUMAP", group.by = "LARRY")  + NoLegend() + ggtitle("LARRY Barcode") + NoAxes()
...
DimPlot(for_clustering, reduction="cloneUMAP", label =T)  + NoLegend() + ggtitle("Clustering") +NoAxes()
DimPlot(for_clustering, reduction="cloneUMAP", group.by = "HasLARRY")
...
```{r larry_barcodes_mature}
to_plot <- sapply(strsplit(full_seurat$other_clones, ';'), function(x){
  length(x)+1
})
to_plot[is.na(full_seurat$LARRY)] <- NA
to_plot <- data.frame(LARRYCounts=to_plot, CellType=full_seurat$CellType)
ggplot(to_plot, aes(x=CellType, y=LARRYCounts, fill=CellType))+geom_boxplot()+plot_theme+
  scale_fill_manual(values=celltypeColors)+theme(axis.text.x=element_text(angle=45, hjust=1))
```
